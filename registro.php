  ...
  <div id="contenedorcentrado">
    <div id="login">
      ...
      <form id="registro-form" action="" method="POST">
        <input type="text" id="nombre" name="nombre" placeholder="Nombre" value="<?php echo isset($_POST['nombre']) ? htmlspecialchars($_POST['nombre']) : ''; ?>">
        <input type="text" id="apellido" name="apellido" placeholder="Apellido" value="<?php echo isset($_POST['apellido']) ? htmlspecialchars($_POST['apellido']) : ''; ?>">
        <input type="text" id="nusuario" name="nusuario" placeholder="Nombre de Usuario" value="<?php echo isset($_POST['nusuario']) ? htmlspecialchars($_POST['nusuario']) : ''; ?>">
        <input type="email" id="correo" name="correo" placeholder="Correo electrónico" value="<?php echo isset($_POST['correo']) ? htmlspecialchars($_POST['correo']) : ''; ?>">
        <input type="password" id="contra" name="contra" placeholder="Contraseña">

        <div class="captcha-wrapper my-3">
          <div class="g-recaptcha" data-sitekey="6LcwclwrAAAAABJq6EPYPNMPg88sQMwm562FzdHW"></div>
        </div>
...
document.getElementById('registro-form').addEventListener('submit', function(event) {
  ...
  let nusuario = document.getElementById('nusuario').value.trim();
  let correo = document.getElementById('correo').value.trim();
  let contra = document.getElementById('contra').value.trim();
  let captcha = grecaptcha.getResponse();

  if (!nombre && !apellido && !nusuario && !correo && !contra) {
  ...
  } else if (!nombre || !apellido || !nusuario || !correo || !contra) {
    event.preventDefault();
    Swal.fire({
      icon: 'warning',
    ...
    Swal.fire({
      ...
      text: 'Por favor, marca que no eres un robot.',
    });
  }
});
</script>
