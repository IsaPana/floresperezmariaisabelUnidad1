...
<head>
  ...
  <title>Sabor y Arte - Accesorios</title>
...
<section class="seccion-categorias" id="velas">
  <h2>Área de Velas</h2>
  <div class="contenedor-categorias">
    <?php 
    $velas = [
      ["corazon.png", "Velas de corazón dorado", "$50.00 MXN"],
      ["cinco.png", "Forma de números color rosa", "$50.00 MXN"],
      ["oso.png", "Velas en forma de Oso", "$200.00 MXN"],
    ];
    foreach ($velas as $vela): ?>
    <div class="categoria">
      <div class="circulo"><img src="img/<?= $vela[0] ?>" alt="<?= $vela[1] ?>"></div>
      <div class="card-info">
        <p class="nombre-pastel"><?= $vela[1] ?></p>
        <p class="precio"><?= $vela[2] ?></p>
       <a href="agregar.php?producto=<?= urlencode($vela[1]) ?>&precio=<?= urlencode($vela[2]) ?>&imagen=img/<?= $vela[0] ?>&descuento=0" class="btn btn-success">Comprar</a>
...
<section class="seccion-categorias" id="utensilios">
  <h2>Utensilios en Promo</h2>
  <div class="contenedor-categorias">
    <?php 
    $utensilios = [
      ["plato.png", "Plato Decorativo de diferentes colores", "$20.00 MXN"],
      ["plato2.png", "Plato Fiesta Temática", "$25.00 MXN"],
      ["utensilios.png", "Set de Vasos y Globos", "$100.00 MXN (Promo)"],
    ];
    foreach ($utensilios as $item): ?>
    <div class="categoria">
      <div class="cinta-descuento">Promo</div>
      <div class="circulo"><img src="img/<?= $item[0] ?>" alt="<?= $item[1] ?>"></div>
      <div class="card-info">
        <p class="nombre-pastel"><?= $item[1] ?></p>
        <p class="precio"><?= $item[2] ?></p>
      <a href="agregar.php?producto=<?= urlencode($item[1]) ?>&precio=<?= urlencode($item[2]) ?>&imagen=img/<?= $item[0] ?>&descuento=15" class="btn btn-success">Comprar</a>
